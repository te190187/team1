...
public class Reservation {
  ...
  public Reservation(YearMonth yearMonth) {
    ...
    for(int i = 1; i < maxDays; i++) {
      ...
      var reservationDate = new ReservationDate(date);

      this.reservationDates.add(reservationDate);
    }
  }

  public void assign(List<StudentEntryBean> studentEntries, List<ClassroomBean> classrooms) {
    ...
    for(var student: studentLessons) {
    ...
    for (var reserveDate : this.reservationDates) {
      reserveDate.assign(studentLessons, classrooms);
    }
  }
}